      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-10 space-y-4">
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">닉네임</label>
          ...
          {/* Password Field */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">비밀번호</label>
            <div className="relative group">
        ...
        {error && (
          <p className="text-[11px] text-red-500 ml-1">{(error as any).message || '회원가입에 실패했습니다.'}</p>
        )}
        ...
        >
          {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : '회원가입'}
        </Button>
