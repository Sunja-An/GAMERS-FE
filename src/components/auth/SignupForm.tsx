...
import { Mail, Lock, User, Hash, FileText, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { useSignup, useDiscordLogin } from '@/hooks/use-auth';
import { useTranslation } from 'react-i18next';
import { createSignupSchema } from '@/lib/validations/user';
...
export function SignupForm() {
  ...
  type SignupFormValues = import('zod').infer<typeof signupSchema>;
  ...
  return (
    <motion.div 
      ...
      className="flex flex-col justify-center w-full max-w-[400px] mx-auto"
    >
      <div className="space-y-1">
        <h2 className="text-3xl uppercase text-[#EEEEF0] font-barlow font-black leading-[1.1] tracking-tight">
        ...
        <p className="text-[#7A7A85] font-inter text-xs font-medium">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-4">
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">{t('auth.signup.fields.email_label')}</label>
          ...
          {/* Password Field */}
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">{t('auth.signup.fields.password_label')}</label>
            <div className="relative group">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="col-span-2 space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">{t('auth.signup.fields.username_label')}</label>
              <div className="relative group">
            ...
            {/* Tag Field */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">{t('auth.signup.fields.tag_label')}</label>
              <div className="relative group">
                <Hash className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4A4A55] group-focus-within:text-[#6EE7B7] transition-colors" />
                <Input 
                  {...register('tag')}
                  placeholder={t('auth.signup.fields.tag_placeholder')} 
                  className="pl-10 bg-[#1A1A20] border-[#24242B] focus:border-[#6EE7B7]/50 h-11 text-sm transition-all"
                />
              </div>
              {errors.tag && (
          ...
          {/* Bio Field */}
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase tracking-wider text-[#7A7A85] ml-1">{t('auth.signup.fields.bio_label')}</label>
            <div className="relative group">
              <FileText className="absolute left-3.5 top-3 w-4 h-4 text-[#4A4A55] group-focus-within:text-[#6EE7B7] transition-colors" />
              <Textarea 
                {...register('bio')}
                placeholder={t('auth.signup.fields.bio_placeholder')} 
                className="pl-10 bg-[#1A1A20] border-[#24242B] focus:border-[#6EE7B7]/50 min-h-[80px] text-sm transition-all resize-none"
              />
            </div>
            {errors.bio && (
        ...
        {error && (
          <p className="text-[10px] text-red-500 ml-1">{(error as any).message || t('auth.signup.error_failed')}</p>
        )}
        ...
        <div className="flex items-center gap-4 pt-2 pb-1">
          ...
          <span className="text-[#4A4A55] font-inter text-[9px] uppercase font-bold tracking-[0.2em]">{t('auth.common.divider_or')}</span>
          <div className="grow h-px bg-[#242428]" />
        </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-[#7A7A85] text-[12px]">{t('auth.signup.footer.has_account')}</span>
          <Link href="/login" className="text-[#6EE7B7] hover:text-white font-bold text-[12px] transition-all hover:tracking-wide">
