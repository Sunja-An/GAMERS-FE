...
export function GameSettingsSection() {
  ...
  const formats = [
    { id: ContestType.TOURNAMENT, name: t('contests.create.game.tournament'), desc: t('contests.create.game.tournament_desc') },
    { id: ContestType.LEAGUE, name: t('contests.create.game.league'), desc: t('contests.create.game.league_desc') },
  ];
  ...
  return (
    <section id="game_settings" className="scroll-mt-32 flex flex-col gap-8 w-full max-w-4xl mx-auto py-12 border-b border-white/5">
      ...
      <div className="flex flex-col gap-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {games.map((game) => (
              <button
                key={game.id}
                type="button"
                onClick={() => setValue('game_type', game.id)}
                className={cn(
              ...
              >
                ...
                {selectedGame === game.id && (
                  <div className="absolute top-4 right-4">
                    <div className="w-5 h-5 rounded-full bg-neon-mint flex items-center justify-center">
        ...
        <div className="flex flex-col gap-6">
          <label className="text-[13px] font-bold text-[#BBBBCB]">
            {t('contests.create.game.select_format')}
            <span className="text-neon-mint ml-1">*</span>
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {formats.map((format) => (
              <button
                key={format.id}
                type="button"
                onClick={() => setValue('contest_type', format.id)}
                className={cn(
                  "relative group flex flex-col items-start gap-2 p-6 rounded-2xl border transition-all duration-300",
              ...
              >
                <span className={cn(
                  "text-sm font-black transition-colors uppercase tracking-widest",
                  selectedFormat === format.id ? "text-neon-mint" : "text-[#EEEEF0]"
                )}>
                  {format.name}
                </span>
                <span className="text-[11px] font-medium text-[#7A7A85] text-left">
                  {format.desc}
                </span>
                {selectedFormat === format.id && (
                  <div className="absolute top-4 right-4">
